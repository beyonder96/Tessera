...
- **Botão Compacto de Nova Tarefa no App:** Substituição do botão largo por um ícone circular minimalista com o símbolo `+`, economizando espaço e alinhando-se com o botão de compartilhamento.
- **Eliminação de Redundância na Web:** A Central de Tarefas e Avisos na Home Web agora é ocultada automaticamente quando o atalho já consta na seção de Acessos Recentes.
- **Notificações Abrangentes no Android:** Correção na regra de notificação do aplicativo móvel para alertar com som prioritário e banner informativo sempre que qualquer nova tarefa ou aviso for cadastrado na Web.

## 🚀 Novidades da Versão 2.0.57

- **Central de Lembretes, Tarefas & Avisos (Web & Android):** Visibilidade mútua completa de tarefas criadas pela Web e pelo app, suporte a menções `@kenned` e alertas direcionados com notificação prioritária no dispositivo móvel (som instantâneo de alta prioridade e pulso duplo).
- **Aba Avisos Web em Rotinas & Hábitos (Zenith):** Nova aba integrada em Rotinas & Hábitos com contador de pendências, aprovação e confirmação em tempo real e conclusão de tarefas.
- **Correção Definitiva de Datas e Extrato Financeiro:** Eliminação do recuo de 1 dia por fuso UTC no seletor de data e unificação do agrupamento de extrato por data efetiva.
- **Cadastro de Contas e Cartões na Web com Lançamento Próprio:** Permite cadastrar contas e cartões próprios na Web com lançamento direto sem necessidade de aprovação, mantendo o fluxo protegido para contas compartilhadas.
- **Tema Claro e Escuro Global na Web:** Suporte refinado à alternância dinâmica de tema claro e escuro em todas as páginas e na tela inicial.

## 🚀 Novidades da Versão 2.0.55

- **Instalação PWA Resiliente no Web Mercado e Web Finanças:** Disponibilização consistente da opção de instalação como aplicativo nos links compartilhados web (`/market/:id` e `/finance/:id`), com ícones PNG de alta fidelidade (192px e 512px) cumprindo requisitos estritos do Web App Manifest, captura antecipada do evento `beforeinstallprompt`, modal explicativo para dispositivos sem suporte ao prompt direto (como iOS Safari) e Service Worker otimizado.
- **Formulário Minimalista de Adição no Web Mercado:** Redesign e simplificação do card de adição ao planejamento na interface web, contendo exclusivamente Nome, Quantidade, Valor e o Botão de Adição, eliminando poluição visual (unidade, seletor de categoria e subtotal estimado) com valores padrão transparentes para compatibilidade contínua com o banco de dados e app móvel.
...
- **Tema Claro e Escuro com Botão Animado no Resumo Web:** Alternância refinada entre tema escuro e claro com microinteração de rotação de 360°, persistência no `localStorage` e respeito às regras de design minimalista.
- **Correção e Sincronização em Tempo Real de Dívidas, Parcelados, Contas e Cartões:** Normalização resiliente com suporte a faturas calculadas dinamicamente e integração total entre o App Android e a Web.
...
- **Robustez na Desserialização do Supabase:** Tratamento defensivo contra arrays e valores vazios ou não inicializados na nuvem, garantindo ausência de erros de formatação monetária e carregamento instantâneo em tempo real.

## 🚀 Novidades da Versão 2.0.49

- **Correção Definitiva do "Disponível para Gastar" ao Zerar Finanças:** Inclusão da tabela de dívidas ativas (`debts`) no método de limpeza do Room DAO (`clearAllDebts()` em `clearAllFinances()`), evitando que parcelas residuais de dívidas deixassem o saldo disponível negativo (como -R$ 300,00) após acionar a ação de zerar.
- **Reset Imediato das Métricas na Nuvem (Supabase):** Redefinição instantânea de `spendableBalance`, `salaryValue`, `committedValue` e `committedPercentage` para 0.0 na sincronização em tempo real e atualização do diálogo de confirmação em Configurações.
